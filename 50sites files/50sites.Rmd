...
checkFile <- "model_relaxedgamma5.rds"
...
emptyFit <- stan(file="model_relaxedgamma5.stan", data = stanDat1, chains = 0)
...
for (i in siteInds) {
  ...
  p2 <- postplot(stanFit) + ggtitle("Sigma Posteriors")
  ...
  # print (or table) mean, median, R_hat, n_eff for each of the 6 params
  datasumm <-summary(stanFit)$summary[1:6,c(1,6,9,10)]
  print(kable(datasumm))
  cat("  \n")
  n_eff_sum <- n_eff_sum + datasumm[,3]
}
mtp <- proc.time() - ptm

...
# Using Model PPTT (previously model 3): add intra-tumoral variance
stanfitPPTT <- function (dataset) {
  ...
  return(stanFitPPTT=stanFitPPTT)
}
...
fitplot <- function(data,stanFit) {
  ...
  sampcoefs <- data.frame(pat = pats, b = sumTemp[pats], c = sumTemp[pats+npats], d = sumTemp[1:nsamp+2*npats])
  patcoefs <- data.frame(pat = 1:npats, b = sumTemp[1:npats], c = sumTemp[1:npats+npats])
  rm(sumTemp)
...
# check for model_PPTT.rds, must add to prevent crash
checkFile <- "model_PPTT.rds"
...
emptyFit <- stan(file="model_PPTT.stan", data = stanDat1, chains = 0)

...
pB <- post_prior_histogram(betaT_full$p50, genemask, function(x) 1.5*dcauchy(x,-0.04051, 0.5799), 0.25, c(-5,5), "BetaT") + theme(aspect.ratio = 1)
...
p_all <- ggarrange(pM,pB,pE,pP,pPT,pT,hjust=0,nrow=2,ncol=3) #,pLP,pLPT,pLT
print(p_all)
...
distance_length <- distanced_data %>%
  filter(!is.na(UCSC_RefGene_Name)) %>%
  select(CHR, UCSC_RefGene_Name, cpgdist, c(sigmaP:sigmaT,score_median)) %>%
  mutate(location_bin = cut(cpgdist, c(seq(-1500, 20000, by = 100)), include.lowest = TRUE)) %>%
  group_by(location_bin) %>%
  summarise(mean_sigmaP = mean(sigmaP), mean_sigmaPT = mean(sigmaPT), mean_sigmaT = mean(sigmaT),  mean_score_median = mean(score_median), n_obs = n(), .groups='drop') %>%

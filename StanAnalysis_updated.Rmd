...
post_prior_histogram <- function(postvals,mask,priorfun,bw,xl,titlestr) {
  ...
  p1 <- ggplot(plot_df, aes(x=val,fill=gene_assoc)) +
    ...
    geom_histogram(aes(y=..density..), binwidth=bw, position="dodge") +
...
pM <- post_prior_histogram(mu_full$p50, genemask, function(x) 1.3*fBG(x,0.3557,-3.0675, 1.3027, 1.1290, 2.4110), 0.25, c(-5,5), "Mu")
pB <- post_prior_histogram(betaT_full$p50, genemask, function(x) 1.5*dcauchy(x,-0.04051, 0.5799), 0.25, c(-5,5), "BetaT")
pE <- post_prior_histogram(sigmaE_full$p50, genemask, function(x) 3.3*dgamma(x,2,2), 0.05, c(0,2), "SigmaE")
pP <- post_prior_histogram(sigmaP_full$p50, genemask, function(x) 1.52*dgamma(x, 2.7693, 7.0029), 0.05, c(0,2), "SigmaP")
pPT <- post_prior_histogram(sigmaPT_full$p50, genemask, function(x) 1.6*dgamma(x, 2.1457, 2.4819), 0.05, c(0,2), "SigmaPT")
pT <- post_prior_histogram(sigmaT_full$p50, genemask, function(x) 0.97*dgamma(x, 3.3643, 10.430), 0.05, c(0,2), "SigmaT")
# log sigmas
...
pP1 <- post_histogram(CpGscore_full$median,genemask,0.05,c(-3,3),expression("Median log("~sigma[P]/sigma[T]~")")) + scale_x_continuous(limits=c(-3,3),breaks=c(-3,-2,-1,0,1,2,3))
...
promoted_data <- read_csv(here(prepdatafolder, "promoted_data.csv"))
cat("done")
```
...
prepped_data %>% select(Name, Relation_to_UCSC_CpG_Island, score_median) %>%
  ...
  mutate(ir = case_when(ir=="" ~ "Sea",
                        str_detect(ir, "^S") ~ str_replace(ir, "^S_", ""),
                        str_detect(ir, "^N") ~ str_replace(ir, "^N_", ""),
                        TRUE ~ ir)) %>%
  mutate(ir = factor(ir, levels = c("Island", "Shore", "Shelf", "Sea"))) %>%
  #ggplot() + geom_density(aes(x=score,y=..scaled..,color=ir)) + theme_classic() + scale_x_continuous(limits=c(-3,3),breaks=c(-3,-2,-1,0,1,2,3)) + ylab("Normalized Density")
  ggplot() + geom_violin(aes(x=ir,y=score,fill=ir),draw_quantiles = 0.5) + theme_classic() + scale_y_continuous(limits=c(-3,3),breaks=c(-3,-2,-1,0,1,2,3))
# ggsave("CpGscore_islands.svg")
...
#   #mutate(fr = factor(fr, levels = c("Island", "Shore", "Shelf", "Sea"))) %>%
...
temp %>%
  ...
  filter(fr %in% keepregions) %>%
  ggplot() + geom_violin(aes(x=fr,y=score,fill=fr),draw_quantiles = 0.5) + theme_classic() + scale_y_continuous(limits=c(-3,3),breaks=c(-3,-2,-1,0,1,2,3))

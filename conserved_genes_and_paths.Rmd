...
permed_data <- read_csv(here("data_final_rg3", "permed_data.csv"))
...
# Gene Set Enrichment with Reactome
tidy_reactome <- function(genes) {
  AnnotationDbi::mapIds(org.Hs.eg.db::org.Hs.eg.db, genes, 'ENTREZID', 'SYMBOL') %>%
    ReactomePA::enrichPathway()
}
# GSEA with KEGG
tidy_kegg <- function(genes) {
  AnnotationDbi::mapIds(org.Hs.eg.db::org.Hs.eg.db, genes, 'ENTREZID', 'SYMBOL') %>%
...
enrich_reactome <-enrichedGenes %>%
  pull(Gene) %>% tidy_reactome()
proc.time()-ptm
cat("Begin KEGG GSEA...")
ptm<-proc.time()
enrich_kegg <-enrichedGenes %>%
  pull(Gene) %>% tidy_kegg()
proc.time()-ptm
...
for (iset in 1:nsets) {
  ...
  v <- cumsum(runsum)
  #score <- max(v) # GSEA score
  score <- max(c(0,v)) + min(c(0,v)) # GSVA score
...
shuffle_genesets <- function(n_geneset, n_rep, data) {
  ...
  for (ishuf in 1:n_rep) {
    ...
    inds = sample(1:ngene, n_geneset, replace = T)
...
cat(sprintf("Shuffling %i set-lengths: 5000x random shuffles (w/ replacement)",length(setlengths)))
ptm<-proc.time()
res <- lapply(setlengths, shuffle_genesets, n_rep = 5000, data = evidence)
proc.time()-ptm

...
cat(sprintf("Load prepped data dir: %s (needs to be pre-processed)...",prepdatafolder))
...
# plot TCGA estimates for mu,betaT,sigmaE,sigmaP,sigmaPT,sigmaT; with prior fits overlaid
...
# plot hierarchical model posterior medians for mu,betaT,sigmaE,sigmaP,sigmaPT,sigmaT; with priors overlaid
...
# plot the two conservations score prob sums (using /log(2) to convert log_e to log_2)
genemask = !(FullAnnotation$UCSC_RefGene_Name=="")
p_scores <- post_histogram(CpGscore_full$median/log(2),genemask,0.05,"CpG Conservation score") +
...
p1 <- permed_data %>% ggplot(aes(x=score_median/log(2))) + geom_histogram(aes(y=..count..), binwidth=0.1, fill="lightgray",color = "grey50") + xlim(c(-1.5,3.5)) + theme_classic() + ylab("Number of genes") + theme(aspect.ratio = 1)
...
prepped_data %>%
  ...
  mutate(score_median = score_median / log(2) ) %>% # convert log_e to log_2
...
prepped_data %>% select(Name, Relation_to_UCSC_CpG_Island, score_median) %>%
  ...
  ggplot() + geom_violin(aes(x=ir,y=score,fill=ir),draw_quantiles = 0.5) + theme_classic() + scale_y_continuous(limits=c(-5,5),breaks=c(-4,-2,0,2,4)) + theme(aspect.ratio=1/4,legend.position = "none")
...
temp <- bind_rows({prepped_data %>% summarize_conservation("any") %>% mutate(region="Gene-Associated")},
  ...
  mutate(cons_in = cons_in/log(2), cons_out = cons_out/log(2)) %>% # change of base e to base 2
...
# multiple regression stats (using log2 score)
...
temp <- prepped_data %>% select(Name, UCSC_RefGene_Group, score_median) %>%
  ...
  mutate(fr = case_when(fr=="" ~ "NA",
                        ...
                        TRUE ~ fr)) %>%
  distinct()
#keepregions = c("TSS1500","Body", "3'UTR","1stExon","TSS200","5'UTR","ExonBnd")
keepregions = c("TSS200","1stExon", "5'UTR","TSS1500","Body","3'UTR","ExonBnd")
temp %>%
  mutate(fr = factor(fr, levels = keepregions)) %>%
  filter(fr %in% keepregions) %>%
  ggplot() + geom_violin(aes(x=fr,y=score,fill=fr), draw_quantiles = 0.5) + theme_classic() + scale_y_continuous(limits=c(-5,5),breaks=c(-4,-2,0,2,4,6)) + theme(aspect.ratio=1/4,legend.position = "none") + geom_hline(yintercept = 0, color="black", alpha=0.2)
...
distance_length %>%
  ...
  labs(title = NULL,
       ...
       caption = "Figure 3D: Average conservation scores of single CpGs as a function of position relative to gene.") +
...
plotGeneScore <- function(geneStr,norm) {
  ...
  ggplot(prepped_data %>% mutate(score_median = score_median/log(2))) + geom_histogram(aes(x=score_median, y=..count../nrow(prepped_data)), binwidth=0.05,fill="lightgray") + ggtitle(sprintf("%s: CpG scores (%i sites)",geneStr, length(CpGscores))) +
...
\subsection{CRC Cancer Genes Databases}
...
perm_temp <- permed_data %>% select(c(UCSC_RefGene_Name, score_median,score_median_v3,n)) %>%
  mutate(score_median2 = score_median/log(2)) # change of base from e to 2
genes_dbs <- rbind(perm_temp %>% mutate(group="all"),
...
# Fig 4B: plot violins (with log2 base change)
pDB <- genes_dbs %>% mutate(group = factor(group, levels = c("all", "AGO", "COSMIC"))) %>%
  ggplot() + geom_violin(aes(x=score_median2,y=group,fill=group), draw_quantiles = 0.5) + scale_x_continuous(limits=c(-2,4),breaks = c(-2,0,2,4)) + theme_classic() + ylab("Database") + xlab("Gene Conservation Score") + geom_vline(xintercept=0, color="black", alpha=0.2) + theme(aspect.ratio=60/156.3,legend.position = "none")
# add hashmarks for each gene (doesn't look great)
marks <- genes_dbs %>% mutate(x=score_median2, y=if_else(group=="COSMIC",3,if_else(group=="AGO",2,1))) %>% select(x,y)
marks2 <- genes_dbs %>% filter(score_median_v3 > 0.95) %>% mutate(x=score_median2, y=if_else(group=="COSMIC",3,if_else(group=="AGO",2,1))) %>% select(x,y)
...
# stats
wilcox.test(x = genes_dbs$score_median2[genes_dbs$group=="COSMIC"], y = genes_dbs$score_median2[genes_dbs$group=="all"], alternative = "greater")
wilcox.test(x = genes_dbs$score_median2[genes_dbs$group=="AGO"], y = genes_dbs$score_median2[genes_dbs$group=="all"], alternative = "greater")
```

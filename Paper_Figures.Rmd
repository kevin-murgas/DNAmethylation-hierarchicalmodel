...
pN <- post_prior_histogram(nu_full$p50, genemask, function(x) 1.7*dcauchy(x,-0.04051, 0.5799), 0.1, c(-5,5), "Nu")
...
p_all <- ggarrange(pM,pN,pE,pP,pPT,pT,pLP,pLPT,pLT,hjust=0,nrow=3,ncol=3)
print(p_all)
rm(pM,pN,pE,pP,pPT,pT,pLP,pLPT,pLT)
```
...
p1 <- permed_data %>% ggplot(aes(x=score_median)) + geom_histogram(aes(y=..count..), binwidth=0.1, fill="lightgray",color = "grey50") + xlim(c(-1.5,3.5)) + theme_classic() + ylab("Number of genes") + theme(aspect.ratio = 1)
...
pDB <- genes_dbs %>% mutate(group = factor(group, levels = c("all", "AGO", "COSMIC"))) %>%
  ggplot() + geom_violin(aes(x=score_median,y=group,fill=group), draw_quantiles = 0.5) + scale_x_continuous(limits=c(-2,4),breaks = c(-2,0,2,4)) + theme_classic() + ylab("Database") + xlab("Gene Conservation Score") + geom_vline(xintercept=0, color="black", alpha=0.2) + theme(aspect.ratio=60/156.3,legend.position = "none")
...
# stats
wilcox.test(x = genes_dbs$score_median[genes_dbs$group=="COSMIC"], y = genes_dbs$score_median[genes_dbs$group=="all"], alternative = "greater")
wilcox.test(x = genes_dbs$score_median[genes_dbs$group=="AGO"], y = genes_dbs$score_median[genes_dbs$group=="all"], alternative = "greater")
```
...
pN <- ggplot(gene_summary) + geom_point(aes(x=n,y=nu), alpha=0.01) + ggtitle("Nu vs N-sites")
...
# arrange each parameter as a subplot
p_all <- ggarrange(pM,pN,pE,pP,pPT,pT,pP1,hjust=0,nrow=3,ncol=3)
print(p_all)
rm(pM,pN,pE,pP,pPT,pT,pP1)
```
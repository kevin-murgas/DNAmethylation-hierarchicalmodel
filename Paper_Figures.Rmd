...
load(here(stanFile))
CpGscore_full = CpGscore_full %>% mutate(mean = mean*2, median=median*2)
...
gened_data <- gened_data %>% mutate(score_median = score_median*2)
prepped_data <- prepped_data %>% mutate(score_median = score_median*2)
permed_data <- permed_data %>% mutate(score_median = score_median*2)
...
p1 <- permed_data %>% ggplot(aes(x=score_median)) + geom_histogram(aes(y=..count..), binwidth=0.1, fill="lightgray",color = "grey50") + xlim(c(-3,7)) + theme_classic() + ylab("Number of genes") + theme(aspect.ratio = 1)

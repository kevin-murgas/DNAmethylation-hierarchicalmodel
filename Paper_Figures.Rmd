...
genes_dbs <- rbind(perm_temp %>% mutate(group="all genes"),
              ...
              perm_temp %>% filter(UCSC_RefGene_Name %in% cardprog) %>% mutate(group="cardprog")
              )
# Fig 4C: plot distribution
pDB <- genes_dbs %>% mutate(group = factor(group, levels = c("all genes", "AGO", "COSMIC", "DEPMAP", "neuron", "cardprog"))) %>%
